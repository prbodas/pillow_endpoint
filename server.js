...
// In-memory conversation store: sessionId -> { system?: string, messages: [{role, content}], updatedAt }
const convoStore = new Map();
const MAX_HISTORY_MESSAGES = 20; // cap total role messages kept per session
      ...
      if (req.method === 'GET') {
        const help = {
          ...
          query: {
            ...
            llm_model: 'Gemini model id (default: gemini-1.5-flash)',
            system: 'optional system prompt',
            reset: 'true to clear history for session',
          },
          note: 'Uses Google Gemini (Generative Language API) with server-side API key.'
        };
    ...
    if (url.pathname === '/transcribe') {
      if (req.method === 'GET') {
        const help = {
          ...
          query: {
            return: 'original | tts | none (default: original)',
            voice: 'TTS voice (when return=tts, default Brian)'
          },
          env: {
            VOSK_MODEL_DIR: 'Path to local Vosk model directory (required)'
          }
        };
        res.writeHead(200, { ...corsHeaders(), 'content-type': 'application/json' });
        return res.end(JSON.stringify(help, null, 2));
      ...
      try {
        ...
        if (!fs.existsSync(conf)) {
          throw new Error(`Missing model files at ${conf}`);
        }
      } catch (e) {
        res.writeHead(500, { ...corsHeaders(), 'content-type': 'text/plain; charset=utf-8' });
      ...
      if (returnMode === 'tts') {
        ...
        try {
          ...
          // update history
          appendHistory(sessionId, systemParam, userText, reply);
          llm = reply; // include assistant text in JSON part
          const { audio, contentType } = await fetchTTS(voice, reply || 'I love my waifu');
          outAudio = audio;
          outType = contentType;
        } catch (e) {
          // fallback to original audio if LLM or TTS fails
          outAudio = body;
          outType = ctypeIn;
        }
      }
...
async function callOpenRouter({ model, system, user, messages }) {
  ...
  // Extract content across possible shapes
  let text = '';
  try {
    ...
    if (typeof msg?.content === 'string') {
      ...
      text = msg.content.map((p) => (typeof p === 'string' ? p : (p?.text || p?.content || ''))).join('').trim();
  ...
  return { raw: json, text, ok: resp.ok, status: resp.status };
}
...
function getSessionId(url, req) {
  return url.searchParams.get('session') || req.headers['x-session-id'] || 'default';
}
...
function buildMessages(sessionId, systemParam, userText) {
  ...
  // Return constructed messages list
  return msgs;
}
...
async function callGemini({ model, system, messages, temperature = 0.9, maxTokens = 256, topP = 0.95 }) {
  ...
  // Flatten all parts' text for the top candidate
...
// Removed HF/OpenRouter/Ollama helpers; using Gemini only

module.exports = {
  // HTTP server (not listening when imported)
  server,
  // Helpers we unit test
  callGemini,
  buildMessages,
  appendHistory,
  getSessionId,
